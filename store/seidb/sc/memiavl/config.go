...
type Config struct {
	// Enable turns on SeiDB state-commit via memIAVL when set in app config.
	// Prefer seidb.enabled for full SeiDB; this flag also sets SeiDB + memiavl SC backend.
	Enable bool `mapstructure:"enable"`

	// ZeroCopy if true, Get/Iterator return slices pointing to mmap-ed snapshot buffers.
	// Zero-copied slices must not be retained beyond the current block execution.
	ZeroCopy bool `mapstructure:"zero-copy"`

	// CacheSize is the per-store LRU cache size on top of the tree (reserved for future use).
