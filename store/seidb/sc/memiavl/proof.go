...
import (
	...
	seiiavl "cosmossdk.io/store/seidb/sc/sei-iavl"
)
...
func convertInnerOps(path seiiavl.PathToLeaf) []*ics23.InnerOp {
...
func pathToLeaf(node Node, key []byte) (seiiavl.PathToLeaf, Node, error) {
	var path seiiavl.PathToLeaf
	...
	for {
		...
		if bytes.Compare(key, node.Key()) < 0 {
			// left side
			right := node.Right()
			path = append(path, seiiavl.ProofInnerNode{
				Height:  height,
				Size:    node.Size(),
